...
get_ipython().system('ls /home/ccc_v1_g_12a11_15981/asn42777_2/asn42779_2/222445/4/work/')
...
# !ls /usr/local/spark/spark-2.2.0-bin-hadoop2.7/bin
# !ls /usr
# !ls .
# !ls /usr/local/hadoop/hadoop-2.7.4/bin/hadoop
# !cat hw2-files.txt
# !locate twitter
...
# Your code here
# raise NotImplementedErrrddFromFileor()
rddFromFile = sc.textFile(file_path[0])
rddFromFile.cache()
count=rddFromFile.count()
#print(count)
...
# YOUR CODE HERE
rdd_tweets=rddFromFile.map(lambda x : safe_parse(x))     .filter(lambda x: x is not None)     .map(lambda x: (x['user']['id_str'],x['text']))
...
# YOUR CODE HERE
#raise NotImplementedError()
#print(rdd_tweets.map(lambda t: t[0]).distinct().take(2))

users_count=rdd_tweets.map( lambda tweet : (tweet[0],1))     .reduceByKey(lambda x,y: x+y)     .count()
...
# YOUR CODE HERE
counts_per_partition=rdd_tweets.map(lambda x : (partition.get(x[0],7),1))     .reduceByKey(lambda x,y: x+y)     .sortByKey()     .collect()
...
# YOUR CODE HERE
num_of_tokens=rdd_tweets.flatMap(lambda x : tok.tokenize(x[1]))     .distinct()     .count()
...
# YOUR CODE HERE
bestset=rdd_tweets.flatMap(lambda x : [(t,x[0]) for t in tok.tokenize(x[1])])     .distinct()     .map(lambda x: (x[0],1))     .reduceByKey(lambda x,y:x+y)     .filter(lambda x : x[1]>100) 

    
num_freq_tokens= bestset.count()  
    
top20 = bestset.sortBy(lambda x:x[1],False)     .take(20)
...
# YOUR CODE HERE
#raise NotImplementedError()
tokens=dict(bestset.collect())

rdd_popular=rdd_tweets.flatMap(lambda x: [(x[0],t)  for t in tok.tokenize(x[1]) if t in tokens])     .distinct()     .map(lambda x: ((partition.get(x[0],7),x[1]),1))     .reduceByKey(lambda x,y : x+y)     .map(lambda x: (x[0],x[1],get_rel_popularity(x[1],tokens[x[0][1]])))     .sortBy(lambda x : (x[0][0],-(x[2]),x[0][1]))     .collect()

popular_10_in_each_group= {}
for i in range(8) :
    groupvals=list(map(lambda x:(x[0][1],x[2]),sorted(rdd_popular, key = lambda x : x[0][0]==i, reverse = True)[:10]))
    popular_10_in_each_group[i]=groupvals
    #print_tokens(groupvals,i)
