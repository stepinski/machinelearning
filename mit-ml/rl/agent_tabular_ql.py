...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    """
    # TODO Your code here
    tmp=q_func[current_state_1, current_state_2, action_index,
           object_index]
    futer = 0 if terminal else GAMMA*(np.max(q_func[next_state_1, next_state_2, :, :]))
    q_func[current_state_1, current_state_2, action_index,
           object_index] = (1-ALPHA)*tmp + ALPHA*(reward + futer)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        nextaction,nextobj=epsilon_greedy(room_idx, quest_idx, q_func, epsilon)
        next_room_desc,next_quest_desc,reward,terminal=framework.step_game(current_room_desc,current_quest_desc,nextaction,nextobj)
        
        next_room_idx=dict_room_desc[next_room_desc]

        if for_training:
            # update Q-function.
            tabular_q_learning(q_func, room_idx, quest_idx, nextaction,nextobj, reward, next_room_idx, room_idx,
                       terminal)
